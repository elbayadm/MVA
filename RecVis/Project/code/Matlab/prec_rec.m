    ...
    if (~hold_fig)
        ...
        if (plot_pr)
            ...
            if (plot_baseline)
                ...
                plot([0 1], [target_ratio target_ratio], '--k'); % Maha
                %plot(log([0 1]), log([target_ratio target_ratio]), '--k'); % Maha
                %semilogx([0 1], [target_ratio target_ratio], '--k'); % Maha
            end
            
            hold on
            hold all
            
            plot([0; tpr], [1 ; prec], 'linewidth',2,'color',col,'linestyle', line); 
            %semilogx([0; tpr], [1 ; prec], 'linewidth',2,'color',col,'linestyle', line); 
            %plot(log([0; tpr]), log([1 ; prec]), 'linewidth',2,'color',col,'linestyle', line); 
            % add pseudo point to complete curve %Maha
            %legend({'PR-rand','PR'},'location','northeast')
            xlabel('recall');
            ylabel('precision');
            % xlabel('log(recall)');
        ...
        if (plot_roc)
            ...
            if (plot_baseline)
                plot([0 1], [0 1], '--k'); %Maha
                %semilogx([0 1], [0 1], '--k'); %Maha
                %plot(log([0 1]), log([0 1]), '--k'); %Maha
            end
            
            hold on;
            hold all;
            
            plot([0; fpr], [0; tpr], 'linewidth',2,'color',col); 
            %semilogx([0; fpr], [0; tpr], 'linewidth',2,'color',col); 
            %plot(log([0; fpr]), log([0; tpr]), 'linewidth',2,'color',col); 
            % add pseudo point to complete curve %Maha
            %legend({'ROC-rand','ROC'},'location','southeast')
            xlabel('false positive rate');
            ylabel('true positive rate');
            % xlabel('log(false positive rate)');
            % ylabel('log(true positive rate)');
            title('ROC curve');
            grid on,
            %axis([0 1 0 1]);
            if (plot_roc && plot_pr)
        ...
        if (plot_pr)
            ...
            plot([0; tpr],[1 ; prec], 'linewidth',2, 'color',col); % add pseudo point to complete curve
            %plot(log([0; tpr]),log([1 ; prec]), 'linewidth',2, 'color',col); 
            %semilogx([0; tpr],[1 ; prec], 'linewidth',2, 'color',col);
        end
