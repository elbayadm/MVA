...
#include "caffe/layers/softmax_noisy_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void SoftmaxNoisyLossLayer<Dtype>::LayerSetUp(
  ...
  shared_ptr<Filler<Dtype> > c_filler(GetFiller<Dtype>(
      this->layer_param_.softmax_noisy_loss_param().matrix_filler()));
  ...
  lr_z_ = this->layer_param_.softmax_noisy_loss_param().update_noise_lr();
}

template <typename Dtype>
void SoftmaxNoisyLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void SoftmaxNoisyLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void SoftmaxNoisyLossLayer<Dtype>::Backward_cpu(
...
template <typename Dtype>
void SoftmaxNoisyLossLayer<Dtype>::BackProp(const Blob<Dtype>& prob,
...
INSTANTIATE_CLASS(SoftmaxNoisyLossLayer);
REGISTER_LAYER_CLASS(SoftmaxNoisyLoss);
