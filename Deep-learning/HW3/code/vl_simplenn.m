...
for i=1:n
  ...
  switch l.type
    ...
    case 'maxout'
      if isfield(l, 'g')
        [res(i+1).x, res(i+1).aux] = vl_nnmaxout(res(i).x,l.g) ;
      else
        [res(i+1).x, res(i+1).aux] = vl_nnmaxout(res(i).x) ;
      end
    case 'custom'
  ...
  %fprintf('Size of %s: %dx%dx%dx%d\n',l.type,size(res(i+1).x)); %Maha
...
if doder
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    switch l.type
      ...
      case 'maxout'
        if isfield(l, 'g')
          res(i).dzdx = vl_nnmaxout(res(i).x,l.g,res(i+1).aux,res(i+1).dzdx) ;
        else
          res(i).dzdx = vl_nnmaxout(res(i).x) ;
        end
      case 'custom'
