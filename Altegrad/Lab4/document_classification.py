...
train = pd.read_csv("../data_classification/webkb-train-stemmed.txt", header=None, delimiter="\t")
print train.shape

test = pd.read_csv("../data_classification/webkb-test-stemmed.txt", header=None, delimiter="\t")
print test.shape
...
for i in xrange(len(all_graphs)):
    ...
    metrics_ = compute_node_centrality(graph)
    ...
    for term in list(set(terms_in_doc)):
        ...
        metrics_term = [tuple[1:5] for tuple in metrics_ if tuple[0]==term][0]
...
print "accuracy degree -Weighted:", metrics.accuracy_score(truth_array,predictions_w_degree)
print "accuracy closeness:", metrics.accuracy_score(truth_array,predictions_closeness)
print "accuracy closeness -Weighted:", metrics.accuracy_score(truth_array,predictions_w_closeness)
