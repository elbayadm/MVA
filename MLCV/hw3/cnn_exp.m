function [net, info] = cnn_exp(classifier, varargin)

if nargin < 1, classifier = 'lin'; end

% Some default  values
opts.dataDir = 'data';
opts.train.gpus=[];
opts.train.learningRate = 0.00 ; % learning rate 
opts.train.weightDecay = 0.0005;
opts.train.momentum = 0.0;
opts.train.numEpochs = 500 ;  
opts.train.batchSize = inf; % how many training examples are processed at a time
opts.train.plotDiagnostics = false;
opts.train.plotStatistics = false;
opts.train.errorFunction = 'binary' ; 
opts.train.conserveMemory = false; % throw away intermediate layer results that are not necessary
opts.train.continue = false; % continue training from snapshot
opts = vl_argparse(opts, varargin) ;
% --------------------------------------------------------------------
%                                                         Prepare data
% --------------------------------------------------------------------

net = cnn_init(classifier) ;
opts.expDir = fullfile(opts.dataDir, classifier);        
switch classifier
    case {'lin','log'}
        [imdb,testData,testLabels] = setupImdb(1000,'linear',[]);
    case 'svm'
        [imdb,testData,testLabels] = setupImdb(1000,'linear','plusminus');
    otherwise
        error('Wrong training mode')
end
...
%                                                                 Test
% --------------------------------------------------------------------
res = vl_simplenn(net,testData);
testPredictions = gather(res(end-1).x(:));
switch classifier
    case {'log','lin'}
...
nerrors = nnz(testPredictions ~= testLabels');
net.trainOpts = opts;
info.test.errors = nerrors;
if opts.train.plotDiagnostics
    % Plot loss across epochs and show #errors
    figure; plot(info.train.objective,'linewidth',2); 
    title(['Loss function: ' num2str(info.train.objective(end)) ...
        ', #errors: ' num2str(nerrors)]);

end

% --------------------------------------------------------------------
function net = cnn_init(classifier, varargin)
% --------------------------------------------------------------------
rng('default'); rng(0) ;
net.layers = {} ; 
switch classifier
    case 'lin'
        net.layers{end+1} = struct('type', 'conv', ...
                           'weights', {{randn(1,1,3,1, 'single'),  []}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
        net.layers{end+1} = struct('type', 'myloss','loss_type','squareloss') ;
    case 'log'
        net.layers{end+1} = struct('type', 'conv', ...
                           'weights', {{randn(1,1,3,1, 'single'),  []}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
        net.layers{end+1} = struct('type', 'myloss','loss_type','logloss') ;
    case 'svm'
        net.layers{end+1} = struct('type', 'conv', ...
                           'weights', {{randn(1,1,2,1, 'single'),  zeros(1,1,'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
        net.layers{end+1} = struct('type', 'myloss','loss_type','hingeloss') ;
    otherwise
        error('train mode not supported!')
end
...
% --------------------------------------------------------------------
function [imdb,testData,testLabels] = setupImdb(nsamples, problem, plusminus)
% --------------------------------------------------------------------
[imdb.images.data,imdb.images.labels] = construct_data(nsamples,'train',problem,plusminus);
[testData, testLabels] = construct_data(nsamples,'test', problem,plusminus);
% matconvnet convolutional and fully connected layers accept inputs
% in the form of m x n x k x d, so we have to reshape the data.
% Also, both the data and the labels have to be single precision.
ndimensions = size(imdb.images.data,1);
imdb.images.data = single(reshape(imdb.images.data, 1,1,ndimensions,nsamples));
imdb.images.labels = single(imdb.images.labels);
imdb.images.set = ones(1,size(imdb.images.data,4));
testData = single(reshape(testData, 1, 1, ndimensions, nsamples));
...
%       net.layers{l}.weights{j} = net.layers{l}.weights{j} + ...
%         thisLR * res(l).dzdw{j} - thisDecay * net.layers{l}.weights{j};
    end
